...
- Use **localStorage** for wallet address, app mode, and session data persistence.
...
2) On connect, capture `publicKey.toBase58()`, persist to localStorage, set app mode REAL.
3) Move to Trade Import step; fetch latest trades via HeliusService + DeriverseTradeService (Lookup reuse) for that address.
4) Cache imported trades in memory for downstream screens (session-only).
...
3) localStorage persistence for address/mode (trades cached in memory).
...
- localStorage writes for address and mode.
