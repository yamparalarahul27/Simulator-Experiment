...
export default function HelpScreen() {
    ...
    return (
        <section className="mx-auto flex max-w-4xl flex-col gap-6 md:gap-8 rounded-lg text-[var(--bs-text-secondary)] backdrop-blur">
            <header className="space-y-3">
                <p className="text-xs md:text-sm uppercase tracking-[0.4em] text-[var(--bs-text-mute)]">Need a hand?</p>
                <h1 className="text-2xl md:text-4xl font-semibold text-[var(--bs-text-primary)]">Help Center</h1>
                <p className="text-base md:text-lg leading-relaxed text-[var(--bs-text-secondary)]">
            ...
            <Accordion
                ...
                className="rounded-lg border border-[var(--bs-border)] bg-[var(--bs-card)] text-base leading-relaxed"
            >
                {FAQ_ITEMS.map((item) => (
                    <AccordionItem key={item.value} value={item.value} className="px-4">
                        <AccordionTrigger className="text-[var(--bs-text-primary)] text-base md:text-lg">
                            {item.title}
                        </AccordionTrigger>
                        <AccordionContent className="text-[var(--bs-text-secondary)] text-sm">
                            {item.body}
                        </AccordionContent>
                    </AccordionItem>
                ))}
            ...
            <footer className="space-y-4 rounded-lg bg-gradient-to-r from-[#00ffff]/15 via-[#00e6e6]/15 to-[#00b3b3]/15 p-4 md:p-6 text-sm leading-relaxed text-[var(--bs-text-primary)]/85">

                <div className="text-center space-y-3 rounded-lg border border-[var(--bs-border)] bg-[var(--bs-bg)]/20 p-4 text-[var(--bs-text-secondary)]">
                    <p>
                        If you have any other question or need help, connect with me on Telegram
                    </p>
                    <div className="flex flex-col sm:flex-row justify-center flex-wrap items-center gap-2 sm:gap-3 text-[var(--bs-text-primary)]">
                        <a
                            href={TELEGRAM_URL}
                            target="_blank"
                            rel="noreferrer"
            ...
            {isQrOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    <div className="relative z-10 w-full max-w-sm rounded-lg bg-[var(--bs-bg)]/95 p-6 text-center shadow-2xl">
                        <button
                            aria-label="Close QR modal"
                            className="absolute right-4 top-4 rounded-lg border border-[var(--bs-border)] p-1 text-[var(--bs-text-tertiary)] transition-colors hover:text-[var(--bs-text-primary)]"
