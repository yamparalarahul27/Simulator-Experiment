        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium transition-all ${selectedTags.length > 0
                                ...
                                : 'border-bs-border bg-bs-card text-bs-text-secondary hover:bg-bs-card-fg'
                                }`}
                        >
                        ...
                        {isFilterOpen && (
                            <>
                                ...
                                />
                                <div className="absolute right-0 mt-2 w-56 bg-bs-bg border border-bs-border shadow-2xl z-50 p-2 glass-morphism">
                                    <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
                                        {availableTags.length === 0 ? (
                                            <p className="text-[10px] text-bs-text-mute p-2 italic">Add annotations to see tags</p>
                                        ) : (
                                            availableTags.map(tag => (
                                                <button
                                                    key={tag}
                                                    onClick={() => toggleTagFilter(tag)}
                                                    className="w-full flex items-center justify-between px-3 py-2 text-xs text-bs-text-secondary hover:bg-bs-card transition-colors group"
                                                >
                                                    <span className={selectedTags.includes(tag) ? 'text-bs-brand' : ''}>
                                                        #{tag}
                                                    </span>
                                                    {selectedTags.includes(tag) && <Check className="h-3 w-3 text-bs-brand" />}
                                                </button>
                                            ))
                                        )}
                                    </div>
                    ...
                    <button
                        ...
                        className="flex items-center justify-center h-10 w-10 rounded-lg border border-bs-border bg-bs-card text-bs-text-secondary transition-colors hover:bg-bs-card-fg"
            ...
            {!loading && trades.length > 0 && filteredTrades.length === 0 && (
                <div className="flex flex-col items-center justify-center min-h-[300px] text-center border border-dashed border-bs-border bg-bs-card p-8">
                    ...
                    <button
                        onClick={() => setSelectedTags([])}
                        className="text-bs-brand hover:text-bs-brand-secondary text-sm font-mono underline underline-offset-4"
                    >
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {loading && Array.from({ length: 6 }).map((_, i) => (
                            <div key={`skeleton-${i}`} className="bg-bs-card border border-bs-border p-6 h-[200px]">
                                <div className="animate-pulse flex flex-col h-full space-y-4">
                                    <div className="flex gap-3">
                                        <div className="w-12 h-12 bg-bs-card"></div>
                                        <div className="space-y-2">
                                            <div className="w-24 h-4 bg-bs-card"></div>
                                            <div className="w-16 h-3 bg-bs-card"></div>
                                        </div>
                                    </div>
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-2 mt-8">
                            <button
                                ...
                                className="rounded-lg border border-bs-border bg-bs-card p-2 text-bs-text-secondary transition-colors hover:bg-bs-card-fg disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                            ...
                            <div className="flex items-center gap-2">
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                                    ...
                                    if (
                                        ...
                                        return (
                                            <span key={page} className="text-bs-text-mute">
                                                ...
                                            </span>
                                        );
                                    }
