...
const CustomTooltip = ({ active, payload, label, showDrawdown }: any) => {
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-bs-bg/90 border border-bs-border p-3 rounded shadow-xl backdrop-blur-md">
                <p className="text-bs-text-tertiary text-xs mb-1">Time: {label}</p>
                {pnlInfo && (
                ...
                {drawdownInfo && drawdownInfo.value > 0 && (
                    <p className="text-bs-error text-sm font-mono mb-1">
                        Drawdown: -{drawdownInfo.value.toFixed(2)}%
                    </p>
                )}
                {ratioInfo && (
                    <p className="text-bs-brand text-sm font-mono">
