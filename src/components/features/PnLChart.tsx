...
const CustomTooltip = ({ active, payload, label, showDrawdown }: any) => {
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-[var(--bs-bg)]/90 border border-[var(--bs-border)] p-3 rounded shadow-xl backdrop-blur-md">
                <p className="text-[var(--bs-text-tertiary)] text-xs mb-1">Time: {label}</p>
                {pnlInfo && (
                ...
                {drawdownInfo && drawdownInfo.value > 0 && (
                    <p className="text-[var(--bs-error)] text-sm font-mono mb-1">
                        Drawdown: -{drawdownInfo.value.toFixed(2)}%
                    </p>
                )}
                {ratioInfo && (
                    <p className="text-[var(--bs-brand)] text-sm font-mono">
