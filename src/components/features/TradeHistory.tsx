...
export default function TradeHistory({ onSwitchToRealData }: TradeHistoryProps = {}) {
  ...
  return (
    <div className="min-h-screen text-[var(--bs-text-primary)] py-8">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Input Mode Switcher */}
        <div className="flex gap-4 border-b border-[var(--bs-border)] pb-2">
          {(['wallet', 'manual'] as InputMode[]).map(mode => (
            <button
              ...
              className={`px-4 py-2 font-semibold uppercase tracking-wide text-sm transition-all ${inputMode === mode
                ? 'text-[var(--bs-text-primary)] border-b-2 border-blue-400'
                : 'text-[var(--bs-text-mute)] hover:text-[var(--bs-text-secondary)]'
                }`}
            >
        ...
        <CardWithCornerShine padding="xs">
          <h2 className="text-xl font-semibold text-[var(--bs-text-primary)] mb-4 text-center">
          ...
          {inputMode === 'manual' ? (
            ...
            <div className="flex flex-col items-center gap-4 text-center py-4 w-full">
              <p className="text-[var(--bs-text-tertiary)] max-w-md">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto justify-center">
                <button
                  ...
                  className="px-6 py-3 bg-[var(--bs-card-fg)] border border-[var(--bs-border)] rounded-lg font-semibold text-[var(--bs-text-primary)] hover:bg-[var(--bs-card-fg)] transition disabled:opacity-50"
                >
                  {connecting
                    ? 'Connecting…'
                    : connected
                ...
                {connected && (
                  <button
                    ...
                    className="px-6 py-3 border border-[var(--bs-border)] rounded-lg font-semibold text-[var(--bs-text-primary)] hover:bg-[var(--bs-card-fg)] transition"
                  >
                    Disconnect
                  </button>
                )}
              ...
              <button
                ...
                className={`px-6 py-3 text-[var(--bs-text-primary)] rounded-lg font-semibold transition disabled:opacity-50 
                  ${hasCachedData ? 'bg-[var(--bs-card-fg)] hover:bg-[var(--bs-border)] border border-[var(--bs-border)]' : 'bg-[var(--bs-info)] hover:bg-[var(--bs-info)]/80'}`}
              >
          ...
          <CardWithCornerShine padding="sm" className="space-y-6">
            ...
            <div className="flex gap-4 border-b border-[var(--bs-border)] pb-2">
              <button
                ...
                className={`px-6 py-3 font-semibold text-sm uppercase tracking-wide transition-all ${activeTab === 'deriverse'
                  ? 'text-[var(--bs-text-primary)] border-b-2 border-blue-400'
                  : 'text-[var(--bs-text-tertiary)] hover:text-[var(--bs-text-primary)]'
                  }`}
              >
                Deriverse Trades
                {deriverseTrades.length > 0 && (
                  <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-[var(--bs-info)]/20 text-blue-300">
            ...
            {activeTab === 'deriverse' && (
              <div className="space-y-4">
                {loadingDeriverse ? (
                  <div className="rounded-lg border border-[var(--bs-border)] bg-[var(--bs-bg)]/80 backdrop-blur-xl p-12 text-center">
                    <div className="flex items-center justify-center">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
                      <span className="ml-3 text-[var(--bs-text-tertiary)]">Parsing Deriverse trades...</span>
                    </div>
                  </div>
                ) : (
                  <>
              ...
              <div>
                {loadingHelius ? (
                  <div className="rounded-lg border border-[var(--bs-border)] bg-[var(--bs-bg)]/80 backdrop-blur-xl p-12 text-center">
                    <div className="flex items-center justify-center">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
                      <span className="ml-3 text-[var(--bs-text-tertiary)]">Fetching transactions...</span>
                    </div>
                  </div>
                ) : transactions.length === 0 ? (
                  <div className="rounded-lg border border-[var(--bs-border)] bg-[var(--bs-bg)]/80 backdrop-blur-xl p-12 text-center max-w-2xl mx-auto">
                    <p className="text-[var(--bs-text-tertiary)] mb-2">Transaction history is not cached.</p>
                    <p className="text-[var(--bs-text-mute)] text-sm mb-6 italic">
                    ...
                    <p className="text-[var(--bs-text-tertiary)] text-sm">
                      Click <span className="text-[var(--bs-text-primary)] font-semibold">"Force Refresh"</span> above to fetch full transaction history from Helius.
                    </p>
                  </div>
                ) : (
                  <div className="rounded-lg border border-[var(--bs-border)] bg-[var(--bs-bg)]/80 backdrop-blur-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-[var(--bs-border)] bg-gradient-to-r from-white/5 to-transparent">
                      <h3 className="text-lg font-semibold text-[var(--bs-text-primary)]">All Transactions (via Helius RPC)</h3>
                      <p className="text-sm text-[var(--bs-text-tertiary)] mt-1">Showing last {transactions.length} transactions for this address</p>
                    </div>
                      ...
                      <table className="w-full text-left">
                        <thead className="sticky top-0 bg-[var(--bs-bg)]/90 backdrop-blur-xl border-b border-[var(--bs-border)]">
                          <tr>
                            ...
                            <th className="px-6 py-4 text-xs font-semibold text-[var(--bs-text-secondary)] uppercase tracking-wider">Status</th>
                            <th className="px-6 py-4 text-xs font-semibold text-[var(--bs-text-secondary)] uppercase tracking-wider text-right">Fee (lamports)</th>
                            ...
                            >
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold backdrop-blur-sm
                                                ${tx.type.includes('Transfer') ? 'bg-[var(--bs-info)]/20 text-blue-300 border border-[var(--bs-info)]/30' :
                                    tx.type.includes('Swap') ? 'bg-[var(--bs-brand-tertiary)]/20 text-[var(--bs-brand-secondary)] border border-[var(--bs-brand-tertiary)]/30' :
                                      tx.type.includes('Deriverse') ? 'bg-[var(--bs-brand-success)]/20 text-green-300 border border-[var(--bs-success)]/30' :
                                        'bg-[var(--bs-text-mute)]/20 text-[var(--bs-text-secondary)] border border-[var(--bs-text-mute)]/30'}`}>
                                  {tx.type}
                                </span>
                              </td>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-[var(--bs-text-primary)] font-mono font-semibold">
        ...
        {!hasSearched && (
          <Accordion
            ...
            className="border border-[var(--bs-border)] rounded-lg bg-[var(--bs-bg)]/80"
          >
            <AccordionItem value="how-it-works" className="px-4">
              ...
              <AccordionContent>
                <div className="text-[var(--bs-text-tertiary)] space-y-2">
                  {inputMode === 'manual' ? (
                    <>
                      ...
                      <p className="mt-4 font-semibold text-[var(--bs-text-primary)]">Steps:</p>
                      <ol className="list-decimal list-inside space-y-1 text-[var(--bs-text-secondary)]">
