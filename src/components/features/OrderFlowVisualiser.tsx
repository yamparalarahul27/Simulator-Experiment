...
const NODE_W = 108;
const NODE_H = 54;
const DEPTH_GAP = 80;    // vertical gap between depth rows
const SIBLING_GAP = 52;  // horizontal gap between nodes in the same row
const PAD = 32;
...
const MARKET_FLOW: FlowGraph = {
    ...
    edges: [
        { from: 'placed', to: 'filled', label: 'instant' },
    ],
};
...
function buildTpSlExtension(tpEnabled: boolean, slEnabled: boolean): { nodes: FlowNode[]; edges: FlowEdge[] } {
    ...
    if (tpEnabled && slEnabled) {
        edges.push({ from: 'tp_order', to: 'sl_order', label: 'OCO cancel', dashed: true });
        edges.push({ from: 'sl_order', to: 'tp_order', label: 'OCO cancel', dashed: true });
    }
    return { nodes, edges };
}
...
function computeLayout(graph: FlowGraph): ComputedLayout {
    // BFS depth assignment — depth becomes the row (y-axis) in TD layout
    ...
    // Group nodes by row (depth level)
    ...
    // Canvas: height grows with depth, width grows with max siblings per row
    ...
    Object.entries(rows).forEach(([rowStr, ids]) => {
        ...
        ids.forEach((id, i) => {
            positions[id] = { x: startX + i * (NODE_W + SIBLING_GAP), y };
        });
    });
    ...
    const layoutEdges: LayoutEdge[] = graph.edges.map(e => {
        ...
        // Forward edges: bottom-center → top-center
        // Backward edges: right-center → right-center (arc on right side)
        return {
            ...e,
...
function edgePath(e: LayoutEdge): string {
    if (e.backward) {
        // Arc on the right side — bulge rightward away from the main flow
        const arcX = Math.max(e.x1, e.x2) + 64;
        return `M ${e.x1},${e.y1} C ${arcX},${e.y1} ${arcX},${e.y2} ${e.x2},${e.y2}`;
    }
    // Vertical S-curve: bottom-center of source → top-center of target
...
function bezierMid(e: LayoutEdge): { x: number; y: number } {
    if (e.backward) {
        return { x: Math.max(e.x1, e.x2) + 70, y: (e.y1 + e.y2) / 2 };
    }
    const dy = Math.max(Math.abs(e.y2 - e.y1) * 0.45, 20);
...
const NODE_STYLE: Record<NodeKind, { fill: string; stroke: string; text: string; sub: string }> = {
    start:    { fill: 'rgba(139,92,246,0.15)',  stroke: 'rgba(139,92,246,0.55)', text: '#c4b5fd', sub: 'rgba(196,181,253,0.45)' },
    state:    { fill: 'rgba(255,255,255,0.04)', stroke: 'rgba(255,255,255,0.18)', text: '#e2e8f0', sub: 'rgba(226,232,240,0.35)' },
    terminal: { fill: 'rgba(34,197,94,0.12)',   stroke: 'rgba(34,197,94,0.45)',  text: '#86efac', sub: 'rgba(134,239,172,0.4)' },
    tp:       { fill: 'rgba(34,197,94,0.10)',   stroke: 'rgba(34,197,94,0.4)',   text: '#4ade80', sub: 'rgba(74,222,128,0.4)' },
    sl:       { fill: 'rgba(239,68,68,0.10)',   stroke: 'rgba(239,68,68,0.4)',   text: '#f87171', sub: 'rgba(248,113,113,0.4)' },
    cancel:   { fill: 'rgba(255,255,255,0.02)', stroke: 'rgba(255,255,255,0.08)', text: '#6b7280', sub: 'rgba(107,114,128,0.4)' },
};

function resolveNodeStyle(kind: NodeKind, side: 'buy' | 'sell') {
    if (side === 'sell' && kind === 'terminal') {
        return { fill: 'rgba(239,68,68,0.12)', stroke: 'rgba(239,68,68,0.45)', text: '#fca5a5', sub: 'rgba(252,165,165,0.4)' };
    }
...
function NodeBox({ node, side }: { node: LayoutNode; side: 'buy' | 'sell' }) {
    ...
    return (
        <g>
            ...
            {edge.label && (() => {
                ...
                return (
                    <g>
                        <rect x={mid.x - labelW / 2 - 3} y={mid.y - 9}
                            width={labelW + 6} height={14}
...
export default function OrderFlowVisualiser({ orderType, side, tpEnabled, slEnabled }: Props) {
    const [zoom, setZoom] = useState(1);
    const graph = useMemo(
        () => buildGraph(orderType, tpEnabled, slEnabled),
        [orderType, tpEnabled, slEnabled]
    );
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <div className="ml-auto flex items-center gap-1">
                    <button
                        ...
                        className="w-6 h-6 flex items-center justify-center text-xs font-mono text-white/40 hover:text-white/70 bg-white/5 border border-white/10 hover:border-white/20 transition-colors"
                    >−</button>
            ...
            <div className="flex-1 overflow-auto">
                ...
                >
                    ...
                    {/* Nodes */}
                    {layout.nodes.map(n => <NodeBox key={n.id} node={n} side={side} />)}
                </svg>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 mt-3 pt-2 border-t border-white/5 flex-wrap">
                {([
                    ...
                    <div key={label} className="flex items-center gap-1">
                        <div className="w-2.5 h-2.5 border flex-shrink-0" style={{ borderColor: color, background: color.replace(')', ', 0.15)').replace('rgba(', 'rgba(') }} />
                        <span className="text-[8px] font-mono text-white/30">{label}</span>
                    </div>
                ))}
                <div className="flex items-center gap-1">
                    <div className="w-5 h-0 border-t border-dashed border-white/20 flex-shrink-0" />
                    <span className="text-[8px] font-mono text-white/30">Loop / OCO</span>
                </div>
            </div>
        </div>
    );
