...
export const TableUI = <T extends Record<string, any>>({
    ...
    return (
        <div
            className={`w-full overflow-hidden rounded-lg border border-bs-border bg-bs-bg/80 backdrop-blur-xl shadow-2xl flex flex-col ${className}`}
            style={{ maxHeight }}
        >
            <div className="overflow-auto flex-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/20">
                <table className="w-full text-left text-sm border-collapse table-fixed">
                    <thead className="sticky top-0 z-10 bg-bs-bg/90 backdrop-blur-md shadow-sm">
                        <tr>
                            {tableColumns.map((col) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <span className="text-bs-text-mute group-hover:text-bs-text-tertiary flex-shrink-0">
                                    ...
                                    {/* Resize Handle */}
                                    <div
                                        className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-bs-brand-tertiary/50 transition-colors z-20"
                    ...
                    <tbody className="divide-y divide-bs-border">
                        {sortedData.length > 0 ? (
                            sortedData.map((row, rowIndex) => (
                                <tr
                                    key={rowIndex}
                                    className="hover:bg-bs-card transition-colors"
                                >
                                    {tableColumns.map((col) => (
                                        <td
                                            key={`${rowIndex}-${col.key}`}
                                            className="px-6 py-4 text-bs-text-secondary overflow-hidden text-ellipsis whitespace-nowrap border-r border-transparent hover:border-bs-border last:border-r-0"
                            ...
                            <tr>
                                <td colSpan={tableColumns.length} className="px-6 py-12 text-center text-bs-text-mute">
                                    No data available
                                </td>
                            </tr>
                        )}
